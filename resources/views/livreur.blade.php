    ...
    $(document).ready(function() {
        $('.table').on('click', '.btn-primary', function() {
            ...
            $.get('{{ route("users.show", ":id") }}'.replace(':id', userId), function(data) {
                $('#user-details-modal').find('.usertile').html(`
                <p>${data.name}</p>
                `);
                $('#user-details-modal').find('.card-text').html(`
                <input type="hidden" class="statusmodal" value="${data.status}">
